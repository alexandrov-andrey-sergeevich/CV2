...
import modules.processing, modules.meta
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        #
        self.original_settings = OriginalSettings()
        self.current_box = self.original_settings
        self.video_paused_by_user = False
        ...
        self.menuSetting.aboutToShow.connect(
            lambda: Setting.openSetting(self)
        ...
        self.menuProcessing.aboutToShow.connect(
            lambda: openProcessingFrame(self)
        )  # открытие формы для обработки изображения
        ...
        self.scaleValue.setText(str(self.scaleSlider.value() / 100))  #  fixed bug
        # анализ
        self.menuAnalize.aboutToShow.connect(
            lambda: Analize.openAnalize(self)
        )  # открытие формы для анализа
        self.verticalFrame.hide()
        # self.verticalFrame.

        self.playIcon = self.style().standardIcon(
            getattr(QStyle.StandardPixmap, "SP_MediaPlay")
        )
        self.pauseIcon = self.style().standardIcon(
            getattr(QStyle.StandardPixmap, "SP_MediaPause")
        )
    ...
    def openFullscreenWindow(self):
        ...
        #  чтобы в полноэкранном режиме можно было пользоваться меню
        ...
        icon = QIcon()
        icon.addPixmap(
            QPixmap("ui\\icons/exit-fullscreen.png"), QIcon.Mode.Normal, QIcon.State.Off
        )
    ...
    def closeFullscreenWindow(self):
        ...
        icon = QIcon()
        icon.addPixmap(
            QPixmap("ui\\icons/fullscreen-12-48.png"),
            QIcon.Mode.Normal,
            QIcon.State.Off,
        )
        self.fullscreenToggle.setIcon(icon)
    ...
    def update_frame(self, image):
        #
        modules.processing.save_current_box_settings(self)
        if self.video_thread is not None and self.current_box != self.original_settings:
            image = image[
                int(self.current_box.y1) : int(self.current_box.y2),
                int(self.current_box.x1) : int(self.current_box.x2),
            ].copy()
            image = modules.meta.fitFrameToWindow(self, image)
        ...
        if self.toggleProcessingButton.isChecked() and (
            self.convertToGrayCheckBox.isChecked()
            or self.binarizationCheckBox.isChecked()
        ):
        ...
        if self.isOriginal and self.source in self.boxes:
            for box in self.boxes[self.source]:
                self.current_scene.addItem(
                    get_rectangle_name(
                        box.name, box.x1 * self.scaleFactor, box.y1 * self.scaleFactor
                    )
                )
                self.current_scene.addItem(
                    get_rectangle(
                        box.x1 * self.scaleFactor,
                        box.y1 * self.scaleFactor,
                        box.x2 * self.scaleFactor,
                        box.y2 * self.scaleFactor,
                    )
                )
        ...
        if self.isOriginal:
            ...
            if len(self.box) == 2:
                name, ok = QInputDialog.getText(
                    self, "Название области", "Введите название:"
                )
                if ok:
                    text = get_rectangle_name(name, self.box[0].x, self.box[0].y)
                    rect = get_rectangle(
                        self.box[0].x, self.box[0].y, self.box[1].x, self.box[1].y
                    )
                    self.current_scene.addItem(text)
                    self.current_scene.addItem(rect)
                    box = Box(
                        name, self.box[0].x, self.box[0].y, self.box[1].x, self.box[1].y
                    )
    ...
    def closeEvent(self, event):
        ...
        if self.isChanged and get_msg_result(
            "Завершение работы", "Сохранить изменения перед выходом?"
        ):
            with open("./boxes.pickle", "wb") as f:
                #  сохранение настроек изображения
                pickle.dump([self.original_settings, self.boxes], f)
